...
class Flask(_PackageBoundObject):
    ...
    def add_url_rule(self, rule, endpoint=None, view_func=None, **options):
        ...
        #把url和试图函数封装到一个Rule对象中
        rule = self.url_rule_class(rule, methods=methods, **options)
        rule.provide_automatic_options = provide_automatic_options

        #把rule对象封装到Map,执行Map的add方法
    ...
    def full_dispatch_request(self):
        ...
        .. versionadded:: 0.7
        """
        #第五步 执行伪中间件  before_first_request方法
        self.try_trigger_before_first_request_functions()
        try:
            #处理个信号，falsk不带信号，需要引入模块
            request_started.send(self)
            #中间件的无返回值 ，直接走下面dispatch_request
        ...
        #第六步 最后把reponse返回，执行finalize_request！
    ...
    def finalize_request(self, rv, from_error_handler=False):
        ...
        try:
            #第七部 把session保存到respons中，执行process_response
    ...
    def process_response(self, response):
        ...
        if not self.session_interface.is_null_session(ctx.session):
            ...
            session_interface = SecureCookieSessionInterface()
            SecureCookieSessionInterface().save_session(self, session, response)
            val = self.get_signing_serializer(app).dumps(dict(session)) 加密之后把session传到response中
        
            此处把session传到response中
            """
        return response
    ...
    def wsgi_app(self, environ, start_response):
        ...
        """
        """
        ctx = self.request_context(environ)执行的：
        把 request，session封装在RequestContext对象中
        实例化RequestContext(self, environ)对象
        def __init__(self, app, environ, request=None):
            self.app = app
            if request is None:
                request = app.request_class(environ)   把request的数据封装在Request类中
            ...
            ctx.push()做的： 
            ctx.push() #此处封装了session      #session是封装在了SecureCookieSession实例化中
            
            self.session = self.app.open_session(self.request)
            return app.session_interface.open_session(self, request)
            ...
            self.session = SecureCookieSession()    #一个继承了字典的类
            第二次过来也是返回SecureCookieSession()   loads解码之后的
        """
        ctx = self.request_context(environ)   #此处封装了request
        #第二步，执行RequestContext的push方法！
        ctx.push()   #push里面把session存在local中
        error = None
        try:
            try:
                #第四步 执行那几个特殊的装饰器 和 视图函数
