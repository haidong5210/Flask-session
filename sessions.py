...
class SecureCookieSessionInterface(SessionInterface):
    ...
    def open_session(self, app, request):
        ...
        """
        val是session的值对应的随机字符串,这里的val与下面save的val是一样的，因为
        他是获取浏览器的session，如果val有值，就去解码 data = s.loads(val, max_age=max_age)，
        解码最后转化成SecureCookieSession一个类似于字典，存在LockStack中
        """
    ...
    def save_session(self, app, session, response):
        ...
        """
        print(val,'save')
        加密session返回到response中返回给浏览器，val是把session加密的结果,一个随机字符串，
        与浏览器上session对应的字符串是相同的，每次加载都会不同，以后加载页面时会把这个
        随机字符串获取到，通过上面的open_session方法，解码之后放在堆栈中，然后引入即可使用session
        """
